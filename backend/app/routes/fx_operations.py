...
<section class="card" style="margin-top:14px"><div class="row" style="justify-content:space-between"><div><h2>Automated App & Trading Oversight Team</h2><p class="muted">Runs with FX. Agents monitor, diagnose and recommend; deterministic risk remains sovereign.</p></div><button onclick="runAgents()">Run team now</button></div><div id="agentHealth" class="badge">Loading agents…</div><div id="agentTeam" class="grid"></div></section>
...
function price(v,asset){if(v===null||v===undefined||v==='')return 'Unknown';let n=Number(v),digits=asset==='Forex'?5:(Math.abs(n)>=100?2:(Math.abs(n)>=1?4:6));return Number.isFinite(n)?n.toFixed(digits):'Unknown'}
