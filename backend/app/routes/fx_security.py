...
</style></head><body><main><a href="/terminal">← FX Terminal</a><h1>Security & Health</h1><p class="lead">The local sentinel starts first. A valid Google Authenticator code is required after every FX restart before scanners, learning, favorite monitoring or signal research can run.</p><div class="grid"><section class="card"><h2>Google Authenticator compatible 2FA</h2><p id="automationState" class="status ACTION">Checking automation lock…</p><p id="authState" class="lead">Checking enrollment…</p><div id="enroll"><button onclick="begin()">Begin secure enrollment</button></div><img id="qr" hidden alt="TOTP enrollment QR code"><p id="secret" class="secret"></p><div class="form"><input id="code" autofocus autocomplete="one-time-code" inputmode="numeric" placeholder="6-digit code or recovery code"><button onclick="verify()">Unlock and start FX</button><button onclick="logout()">Lock browser</button></div><p id="message" class="lead"></p><pre id="recovery"></pre></section><section class="card"><h2>Autopilot sentinel</h2><p class="lead">Checks every minute. It can report or block unsafe operation; it cannot enable live trading or change risk limits.</p><div id="checks"></div></section></div></main><script>
let enrolled=false;async function load(){const s=await fetch('/api/security/totp/status').then(r=>r.json());enrolled=!!s.enrolled;const gate=s.automation||{};const gateNode=document.getElementById('automationState');gateNode.textContent=gate.unlocked?'AUTOMATION UNLOCKED · scanners and learning active':'AUTOMATION LOCKED · enter your current code to start scanners and learning';gateNode.className='status '+(gate.unlocked?'PASS':'ACTION');document.getElementById('authState').textContent=enrolled?'2FA is enrolled. Enter a current code after every FX restart to unlock automation and this browser for 8 hours.':'2FA is not enrolled. Begin setup, scan the QR code, then confirm a current code.';document.getElementById('enroll').hidden=enrolled;const h=await fetch('/api/security/sentinel').then(r=>r.json());const root=document.getElementById('checks');root.replaceChildren();(h.checks||[]).forEach(c=>{const row=document.createElement('div');row.className='row';const text=document.createElement('div');text.textContent=c.name+' — '+c.detail;const state=document.createElement('span');state.className='status '+c.status;state.textContent=c.status;row.append(text,state);root.append(row)})}
...
def totp_status():
    return {
        ...
        "retry_after": totp_service.retry_after(),
        "automation": automation_runtime_gate.status(),
    }
...
async def totp_confirm(req: CodeRequest, response: Response):
    try:
    ...
    sentinel.run_once()
    automation = await automation_runtime_gate.unlock("TOTP_ENROLLMENT")
    return {"ok": True, **result, "automation": automation}


@router.post("/api/security/totp/login")
async def totp_login(req: CodeRequest, response: Response):
    ...
    _set_session(response)
    automation = await automation_runtime_gate.unlock("TOTP_LOGIN")
